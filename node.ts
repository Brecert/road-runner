...
export class Node<V> {
  ...
  search(this: Node<V>, searchPath: string): SearchResult<V> {
    ...
    while (true) {
      ...
      if (
        searchPathLength > n.pathLength &&
        searchPath.slice(0, n.pathLength) === n.path
      ) {
        ...
        if (n.wildChild) {
          ...
          while (
            end < searchPathLength &&
            searchPath.charCodeAt(end) !== SLASH_CODE
          ) {
            end++;
          }
  ...
  private processWildcard(
    ...
    const isMatch = childPath.length >= this.path.length &&
      ...
      (this.path.length >= childPath.length ||
        childPath[this.path.length] === "/");
    if (isMatch) {
    ...
    throw new Error(
      `'${pathSeg}' in new path '${fullPath}' conflicts with existing wildcard '${this.path}' in existing prefix '${prefix}'`,
    );
  }
  ...
  private createNode(
    ...
    if (this.wildChild) {
      this.children[0].processWildcard(
        fullPath,
        nextChildPath,
        handle,
        numParams - 1,
